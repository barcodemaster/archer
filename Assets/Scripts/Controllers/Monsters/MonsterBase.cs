...
public abstract class MonsterBase : MonoBehaviour, IDamageable
{
	...
	private float _lastContactDamageTime = float.NegativeInfinity;
	...
	protected virtual void Start()
	{
		...
		float groundY = StageManager.Instance?.TileMap != null
			? StageManager.Instance.TileMap.GridToWorld(0, 0).y
			: 0f;
		transform.position = new Vector3(transform.position.x, groundY + _snapToGroundOffset.y, transform.position.z);
	...
	private void PlayHitSquash()
	{
		...
		_hitScaleCoroutine = StartCoroutine(HitSquashCoroutine());
	}

	private IEnumerator HitSquashCoroutine()
	{
		GameConfig cfg = GameConfig.Instance;
		Vector3 squash = new Vector3(cfg.hitSquashScaleXZ, cfg.hitSquashScaleY, cfg.hitSquashScaleXZ);
		Vector3 stretch = new Vector3(cfg.hitStretchScaleXZ, cfg.hitStretchScaleY, cfg.hitStretchScaleXZ);

		try
		{
			// Phase 1: Squash
			float t = 0f;
			while (t < cfg.hitSquashDuration)
			{
				if (_modelTransform == null) yield break;
				_modelTransform.localScale = Vector3.Lerp(Vector3.one, squash, t / cfg.hitSquashDuration);
				t += Time.deltaTime;
				yield return null;
			}
			...
			// Phase 2: Stretch
			t = 0f;
			while (t < cfg.hitStretchDuration)
			{
				if (_modelTransform == null) yield break;
				_modelTransform.localScale = Vector3.Lerp(squash, stretch, t / cfg.hitStretchDuration);
				t += Time.deltaTime;
				yield return null;
			}
			...
			// Phase 3: Recover
			t = 0f;
			while (t < cfg.hitRecoverDuration)
			{
				if (_modelTransform == null) yield break;
				_modelTransform.localScale = Vector3.Lerp(stretch, Vector3.one, t / cfg.hitRecoverDuration);
				t += Time.deltaTime;
				yield return null;
			}
		}
	...
	protected IEnumerator Patrol(float radius, float nextDelay, float speed)
	{
		State = EState.Move;
		...
		for (int i = 0; i < 5; i++)
		{
			...
			Vector3 candidate = transform.position + new Vector3(Mathf.Cos(angle) * distance, 0f, Mathf.Sin(angle) * distance);

			if (tileMap == null || tileMap.CanPass(candidate, _passability != null ? _passability.PassFlags : ETilePassFlag.Walk))
			{
				newPos = candidate;
				found = true;
				break;
	...
	public void TryContactDamage(PlayerController player)
	{
		...
		if (Time.time - _lastContactDamageTime >= _contactDamageCooldown)
		{
